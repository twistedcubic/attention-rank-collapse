...
class Trainer:
    ...
    def train(self, model_path: Optional[str] = None):
        ...
        config = model.config
        model_path = 'modelNLP{}d{}_{}_{}.pt'.format(config.num_attention_heads, config.num_hidden_layers, config.hidden_size, int(self.args.num_train_epochs))
        ...
        ### Path inference ###
        args = self.args_sort if self.args_sort is not None else sort.parse_args()
        ...
        while path_len <= args.depth:
            ...
            for _ in range(n_repeat):
                ...
                for _ in range(args.n_paths):
                    path_idx_l.append(convex_hull.create_path(path_len, args, all_heads=args.all_heads ))
                #n_step = 0
                train_dataloader_seq = self.get_train_dataloader(sequential=True)
                epoch_iterator = tqdm(train_dataloader_seq, desc="CE Iteration", disable=not self.is_local_process_zero())
                for step, inputs in enumerate(epoch_iterator):
        ...
        plot.plot_scatter(plot_arg, fname='nlpTok{}d{}_{}'.format(config.num_attention_heads, config.num_hidden_layers, config.hidden_size ) )
        plot_res_path = 'nlp_res{}d{}_{}path{}'.format(config.num_attention_heads, config.num_hidden_layers, config.hidden_size, args.n_paths ) #'nlp_res{}.pt'.format(args.n_paths)
        torch.save({'auc_ar':auc_ar, 'std_ar':auc_std_ar}, plot_res_path )
        pdb.set_trace()

        plot_res_ar = np.zeros((4, metric_ar.shape[-1]))
        plot_std_ar = np.zeros((4, metric_ar.shape[-1]))
        for i, pathLen in enumerate([5, 20]):
            results = torch.load('nlp_res{}d{}_{}path{}'.format(config.num_attention_heads, config.num_hidden_layers, config.hidden_size, pathLen ) )
