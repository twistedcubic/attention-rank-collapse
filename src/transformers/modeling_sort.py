...
#no_bias used to check consistency between theory and actual
#and to separate out paths. no_bias does not mean no bias.
no_bias = True
#no_bias = False

do_layernorm = True
#remove skip/residual connections
no_skip = 1 #True, any number >0 means no skip at some point
no_skip = 0

project_residual = True
project_residual = False
...
#no intermediate or bert output layers. Accomplished by no bias
...
class BertSelfAttention(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        if no_bias:
            #self.query = torch.nn.Parameter(torch.zeros(config.hidden_size, self.all_head_size).normal_(0, 1)) #nn.Linear(config.hidden_size, self.all_head_size)
            #self.key = torch.nn.Parameter(torch.zeros(config.hidden_size, self.all_head_size).normal_(,0 1)) #nn.Linear(config.hidden_size, self.all_head_size)                        
            self.value = torch.nn.Parameter(torch.zeros(config.hidden_size, self.all_head_size)) #nn.Linear(config.hidden_size, self.all_head_size)
    ...
    def forward(
        ...
        #pdb.set_trace() np.linalg.norm(self.key.weight.cpu().numpy(),1)*np.linalg.norm(self.query.weight.cpu().numpy(),1)*plot.composite_norm(self.value[:22] )
        if encoder_hidden_states is not None:
            ...
            if no_bias:
                ...
                if path_idx_l is not None and path_idx_l[self.layer_idx] != -1:
                    ...
                    #pdb.set_trace()
                    mixed_value_layer = torch.matmul(hidden_states, self.value[start_idx : start_idx+head_sz ].t()) + self.value_bias[start_idx : start_idx+head_sz ] #self.value.t()) 
                else:                        
                    mixed_value_layer = torch.matmul(hidden_states, self.value.t()) + self.value_bias #self.value.t()) 
            else:
        ...
        if path_idx_l is not None and path_idx_l[self.layer_idx] != -1:
            #pdb.set_trace()
            context_layer = torch.matmul(attention_probs[:, path_idx_l[self.layer_idx]:path_idx_l[self.layer_idx]+1, :, :], value_layer)
...
class BertSelfOutput(nn.Module):
    def __init__(self, config, layer_idx=None):
        super().__init__()
        if no_bias:
            self.dense = nn.Parameter(torch.zeros(config.hidden_size, config.hidden_size) ) #nn.Linear(config.hidden_size, config.intermediate_size)
        ...
        self.config = config
        if separate_LN:
            self.LayerNormX = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
    ...
    def forward(self, hidden_states, input_tensor, path_idx_l=None):
        if no_bias:
            if path_idx_l is not None and path_idx_l[self.layer_idx] != -1:
                ...
                start_idx = path_idx_l[self.layer_idx]*head_sz
                #pdb.set_trace()
                hidden_states = torch.matmul(hidden_states, self.dense.t()[start_idx : start_idx+head_sz ]) + self.dense_bias #self.value.t()) 
            else:
        ...
        def do_LN(hidden_states, input_tensor):
            if project_residual:
                #pdb.set_trace()
                scales = ((input_tensor**2).sum([1,2]) / (hidden_states**2).sum([1,2])).sqrt()
                hidden_states = hidden_states * scales.view(-1, 1, 1)
                
            if outer_LN:
                hidden_states = self.LayerNorm(hidden_states) + input_tensor
            elif separate_LN:
                hidden_states = self.LayerNorm(hidden_states) + self.LayerNormX(input_tensor)
            else:
                if path_idx_l is not None and hasattr(self.config, 'no_sub_path') and self.config.no_sub_path:
                    #pdb.set_trace()
                    hidden_states = BertLayerNorm(self.config.hidden_size, eps=self.config.layer_norm_eps, elementwise_affine=False)(hidden_states)
                else:
                    hidden_states = self.LayerNorm(hidden_states + input_tensor)
        ...
        if no_bias:            
            ...
            if do_layernorm:
                ...
                if hasattr(self.config, 'compute_alpha') and self.config.compute_alpha:
                    ...
                    for ii in range(total_tok):
                        Nx += plot.composite_norm(input_tensor[ii])
                        Nmhsa += plot.composite_norm(hidden_states[ii])
            ...
            if no_skip > 0 and self.config.cur_layer % no_skip == 0:
                hidden_states = self.LayerNorm(hidden_states)
                #hidden_states = self.LayerNorm(hidden_states + input_tensor)
            else:
                if project_residual:
...
class BertAttention(nn.Module):
    ...
    def forward(
        ...
        self_outputs = self.self(
            hidden_states, attention_mask, head_mask, encoder_hidden_states, encoder_attention_mask, output_attentions, path_idx_l=path_idx_l
        )
        #pdb.set_trace()
        #if path_idx_l is not None:
        #    pdb.set_trace()
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
        if False: #no_bias:
            self.dense = nn.Parameter(torch.zeros(config.intermediate_size, config.hidden_size).normal_(0,1)) #nn.Linear(config.hidden_size, config.intermediate_size)
        else:
    ...
    def forward(self, hidden_states):
        if False: #no_bias:
            hidden_states = torch.matmul(hidden_states, self.dense.t()) #self.dense.t()) 
        else:
            hidden_states = self.dense(hidden_states)
...
class BertOutput(nn.Module):
    def __init__(self, config):
        ...
        #
        if False: #no_bias:
            self.dense = nn.Parameter(torch.zeros( config.hidden_size, config.intermediate_size).normal_(0, 1))
            if do_layernorm:
                self.LayerNorm = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        else:
    ...
    def forward(self, hidden_states, input_tensor):
        if False: #no_bias:
            hidden_states = torch.matmul(hidden_states, self.dense.t())
        else:
            hidden_states = self.dense(hidden_states)
            hidden_states = self.dropout(hidden_states)
        if no_bias:
            ...
            if no_skip>0 and self.config.cur_layer % no_skip == 0:
                ...
                if outer_LN:
                    ...
                    hidden_states = self.LayerNorm(hidden_states) + self.LayerNorm(input_tensor)
                else:
                    hidden_states = self.LayerNorm(hidden_states + input_tensor)
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        #pdb.set_trace()   #plot.composite_norm(hidden_states -hidden_states.mean(0))
        ...
        #check here to see if hidden state output is equivalent to simply multiplying left by iterated att mx, and right by W_val, W_out, and layer norm non linearity
        ...
        #if False and no_bias:
        if False and no_bias and (self.config.cur_step+1) % 600 == 0:
            #if no_bias:
            ...
            for i, bertlayer in enumerate(self.layer):                
                ...
                for j in range(num_heads):
                    ...
                    cur_right_mx = torch.matmul(bertlayer.attention.self.value[j*dim_per_head : (j+1)*dim_per_head].t(), bertlayer.attention.output.dense.data.t()[j*dim_per_head : (j+1)*dim_per_head ]).detach()#.clone()
                ...
                '''
                hid_states = torch.matmul(bertlayer.self.att_probs, hid_states)
                hid_states = torch.matmul(hid_states, bertlayer.self.value.weights)
                hid_states = torch.matmul(hid_states, bertlayer.self.output.weights)
                '''
                #can add bertoutput and bertintermediate layers here, along with layernorm

            left_res_mx_l = []
            right_res_mx_l = []
            init_left = torch.eye(len(left_mx_l[0][0]), device=left_mx_l[0][0].device)
            init_right = torch.eye(len(right_mx_l[0][0]), device=right_mx_l[0][0].device) #torch.ones_like(right_mx_l[0][0] )
            ...
            diff = torch.sqrt( ((hid_states-hidden_states )**2).sum() )
            print('DIFF between layers forward and using paths {}'.format(diff))
            #pdb.set_trace()
            del diff
        #elif no_skip or True: #or False:
        #elif (no_skip or cur_path_idx_l is not None) and False:
        elif (no_skip or cur_path_idx_l is not None) and hasattr(self.config, 'do_rank') and self.config.do_rank:
        #elif no_skip and False:
            ...
            for states in all_hidden_states:
                ...
                ranks = [torch.matrix_rank(mx, tol=.5).item() for mx in states]
