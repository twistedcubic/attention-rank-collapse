...
def parse_args( ):
    ...
    parser = NoErrArg()
    #parser.add_argument("--verbose", dest='verbose', action='store_const', default=False, const=True, help='Print out verbose info')
    ...
    parser.add_argument("--no_repeat", action='store_const', default=False, const=True, help='whether have repeat chars when generating sorting data')
    parser.add_argument("--train_as_eval", action='store_const', default=False, const=True, help='train as eval')
    parser.add_argument("--no_sub_path", action='store_const', default=False, const=True, help='no skip connection during inference')
    parser.add_argument("--do_rank", action='store_const', default=False, const=True, help='do rank computation')
    parser.add_argument("--compute_alpha", action='store_const', default=False, const=True, help='compute alpha ratio in LN')
    parser.add_argument("--circle_skip", action='store_const', default=False, const=True, help='apply skip connection to circle experiment')
    parser.add_argument("--do_mlp", action='store_const', default=False, const=True, help='whether to do MLP')
    parser.add_argument("--ffn2", action='store_const', default=False, const=True, help='whether to do 2nd feedforward layer in initial embeddings module')
    parser.add_argument("--do_train", action='store_const', default=False, const=True, help='train model even if cached ones exist')
    parser.add_argument("--all_heads", action='store_const', default=False, const=True, help='use all heads / paths')
    parser.add_argument("--bert", action='store_const', default=True, const=True, help='use models defined in modeling_sort.py rather than sanity check models. Currently tautological.')
    parser.add_argument("--res_file", default='results.txt', type=str, help='path to results file ')
...
class SortingModel(nn.Module):
    def __init__(self, args):
        ...
        self.transformer = nn.Transformer(d_model=args.hidden_dim, nhead=args.width, num_encoder_layers=0, num_decoder_layers=args.depth)
...
def main_bert(args):
    ...
    config = BertConfig(vocab_size=args.n_vocab,
    ...
    model_path = 'modelsort{}d{}_{}h{}label{}.pt'.format(args.width, args.depth, args.seq_len, args.hidden_dim, args.n_vocab)
    ...
    while path_len <= args.depth:
        ...
        for data in eval_loader:
            ...
            res_str = 'path_len {} avg_mismatched_pairs {} eval_loss {} complete_match {} partial_match {}'.format(path_len, score_ar.mean(), eval_loss_ar.mean(), comp_match_ar.mean(), \
                                                                                                                     part_match_ar.mean())
...
def main(args):
    ...
    for data in eval_loader:
        ...
        with torch.no_grad(): 
            ...
            print('\navg mismatched pairs {} eval loss {}eval complete m {} eval partial match {}'.format(score, eval_loss, complete_match, \
                                                                                                        partial_match))
        with open(args.res_file, 'a') as f:
            f.write('~{}  \n'.format(args ))
