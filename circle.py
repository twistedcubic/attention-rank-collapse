...
class CircleDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (torch.stack((self.all_data[idx], self.all_data[idx+len(self)]), 0), torch.stack((self.all_data[idx+1], self.all_data[idx+1+len(self)]), 0) )
...
def gen_data(n_data, args):
    """
    Generate convex hull data. random points, generate binary labels
    """
...
def main_bert(args):
    ...
    model_path = 'modelCircle{}d{}_h{}n{}{}{}.pt'.format(args.width, args.depth, args.hidden_dim, args.n_train_data,\
    ...
    config = BertConfig(vocab_size=2, #args.n_vocab,
                        no_embed=True,
    ...
    if args.circle_skip:
        LN = lambda x:x #x : .5 * x/torch.sqrt((x**2).sum(-1)).unsqueeze(-1) #torch.nn.LayerNorm(2, eps=1e-5, elementwise_affine=False)
    else:
        LN = lambda x : x
    config.cur_step = 0
    ...
    plot_arg.title = '{} hidden dim, {} skip, {} MLP' \
            .format(args.hidden_dim, 'w/' if args.circle_skip else 'no', 'w/' if args.do_mlp else 'no')
    #plot_arg.title = 'Training trajectories of point sequences'
    plot.plot_scatter(plot_arg, fname='circle{}d{}_{}{}{}'.format(args.width, args.depth, args.hidden_dim,  'mlp' if args.do_mlp else '',\
                                                                  'skip' if args.circle_skip else ''),\
                      xlim=[-.56,.56], ylim=[-.56,.56])
  ...
  #                                                       'mlp' if args.do_mlp else '', 'skip' if args.circle_skip else '')
    ...
    print('press to save to {}'.format(res_path))
    pdb.set_trace()
    torch.save(res_dict, res_path)
    res_path1 = 'resCircle{}d{}_h{}n{}{}{}'.format(args.width, args.depth, args.hidden_dim, args.n_train_data,\
        'mlp' if args.do_mlp else '', 'skip' if args.circle_skip else '')
    dir_path = 'results_circle'
...
def create_path(path_len, args, all_heads=False):
    ...
    ###
    '''
    if path_idx_l[-1] == args.width:
        path_idx_l[-1] = max(0, path_idx_l[-1]-1)
    if path_idx_l[0] == args.width:
        path_idx_l[0] = max(0, path_idx_l[0]-1)
    #path_idx_l[-2] = 0    
    '''
    ###
    if all_heads:
