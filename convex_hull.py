...
def main_bert(args):
    ...
    config = BertConfig(vocab_size=2, #no embeddings for convex hull.
                        no_embed=True,
        ...
        for data in eval_loader:
            ...
            for i in range(n_repeat):
                with torch.no_grad():
                    ...
                    #score_ar[i] = evaluate_metric_auc(pred.view(-1, args.num_labels), labels.view(-1), args)
                    soft_match_ar[i] = evaluate_metric_softmax_match(pred.view(-1, args.num_labels), labels.view(-1), args)
            res_str = 'path_len {} eval_loss {} token_acc {}'.format(path_len, eval_loss_ar.mean(), soft_match_ar.mean()) #, comp_match_ar.mean(), 
    ...
    plot_res_path = 'convex_hull_res{}d{}_{}_{}.pt'.format(args.width, args.depth, args.hidden_dim, args.n_paths) #'convex_hull_res{}.pt'.format(args.n_paths)
    ...
    for i, pathLen in enumerate([5, 20]):
        ...
        results = torch.load('convex_hull_res{}d{}_{}_{}.pt'.format(args.width, args.depth, args.hidden_dim, pathLen) )
    ...
    plot_arg.y_label = 'Token Prediction Accuracy'
    #plot_arg.title = 'AUC vs Path Length for Convex Hull'    
    plot.plot_scatter(plot_arg, fname='convex_hull{}d{}_{}l{}multi'.format(args.width, args.depth, args.hidden_dim, args.seq_len), loc='best', bbox=(0.5, 0.4, 0.5, 0.5)) #plt.legend(loc='best', bbox_to_anchor=(0.5, 0.4, 0.5, 0.5))
